    ...
    const handleMetaMaskLogin = async () => {
        if (typeof window.ethereum !== 'undefined') {
            try {
                ...
                console.log("MetaMask login successful. User address:", userAddress);
                // Handle the user login, e.g., send userAddress to your backend
                            ...
                            <form className="w-full">
                                {title === "Create a new account" ? (
                                    <>
                                        ...
                                        <input name="fullName" type="text" required className="block w-full p-2.5 border border-slate-300 rounded bg-gray-50 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#333]" placeholder="Full Name" />
                                        <label htmlFor="phoneNumber" className="sr-only">Phone Number</label>
                                        <input name="phoneNumber" type="tel" required className="block w-full p-2.5 border border-slate-300 rounded bg-gray-50 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#333]" placeholder="Phone Number" />
                                        <label htmlFor="referralCode" className="sr-only">Referral Code</label>
                                        <input name="referralCode" type="text" className="block w-full p-2.5 border border-slate-300 rounded bg-gray-50 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#333]" placeholder="Referral Code (optional)" />
                                    </>
                                ) : (
                                    <>
                                        <label htmlFor="email" className="sr-only">Email</label>
                                        <input name="email" type="email" required className="block w-full p-2.5 border border-slate-300 rounded bg-gray-50 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#333]" placeholder="Email" />
                                    </>
                                )}
                                <label htmlFor="password" className="sr-only">Password</label>
                                <input name="password" type="password" required className="block w-full p-2.5 border border-slate-300 rounded bg-gray-50 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#333]" placeholder="Password" />
                                <button type="submit" className="inline-flex w-full items-center justify-center rounded border border-slate-300 bg-black p-2 text-sm font-medium text-white outline-none focus:ring-2 focus:ring-[#333] focus:ring-offset-1">Submit</button>
                            </form>
                        </div>
                    </div>
                </div>
